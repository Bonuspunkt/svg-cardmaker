python generate_cards.py --input ./card-db

#python collect_and_print.py \
#  --cards "./out_cards" \
#  --add "Potion_of_Healing=1" \
#  --add "Potion_of_Greater_Healing=1" \
#  --add "Potion_of_Superior_Healing=1" \
#  --add "Potion_of_Supreme_Healing=1" \
#  --add "Club=1" \
#  --add "Dagger=1" \
#  --add "Greatclub=1" \
#  --add "Handaxe=1" \
#  --add "Javelin=1" \
#  --add "Light_Hammer=1" \
#  --add "Mace=1" \
#  --add "Quarterstaff=1" \
#  --add "Sickle=1" \
#  --add "Spear=1" \
#  --add "Crossbow_Light=1" \
#  --add "Dart=1" \
#  --add "Shortbow=1" \
#  --add "Sling=1" \
#  --add "Battleaxe=1" \
#  --add "Flail=1" \
#  --add "Glaive=1" \
#  --add "Greataxe=1" \
#  --add "Greatsword=1" \
#  --add "Halberd=1" \
#  --add "Lance=1" \
#  --add "Longsword=1" \
#  --add "Maul=1" \
#  --add "Morningstar=1" \
#  --add "Pike=1" \
#  --add "Rapier=1" \
#  --add "Scimitar=1" \
#  --add "Shortsword=1" \
#  --add "Trident=1" \
#  --add "War_Pick=1" \
#  --add "Warhammer=1" \
#  --add "Whip=1" \
#  --add "Blowgun=1" \
#  --add "Crossbow_Hand=1" \
#  --add "Crossbow_Heavy=1" \
#  --add "Longbow=1" \
#  --add "Amulet_of_Myrdell=5" \
#  --add "Communication_Stone=2" \
#  --add "Bulwark_of_Enduring_Mercy=1" \
#  --add "Giant_Slayer_Warhammer=1" \
#  --add "Harvestwarden_Orb=3" \
#  --add "Personal_Space=3" \
#  --crop 

#python collect_and_print.py \
#  --cards "./out_cards" \
#  --add "Potion_of_Healing=6" \
#  --add "spell_Guidance=1" \
#  --add "spell_Mending=1" \
#  --add "spell_Sacred_Flame=1" \
#  --add "spell_Bless=1" \
#  --add "spell_Cure_Wounds=1" \
#  --add "spell_Bane=1" \
#  --add "spell_Command=1" \
#  --add "spell_Create_or_Destroy_Water=1" \
#  --add "spell_Detect_Poison_and_Disease=1" \
#  --add "spell_Guiding_Bolt=1" \
#  --add "spell_Healing_Word=1" \
#  --add "spell_Inflict_Wounds=1" \
#  --add "spell_Protection_from_Evil_and_Good=1" \
#  --add "spell_Sanctuary=1" \
#  --add "spell_Shield_of_Faith=1" \
#  --add "spell_Purify_Food_and_Drink=1" \
#  --add "spell_Detect_Evil_and_Good=1" \
#  --add "spell_Detect_Magic=1" \
#  --add "spell_Healing_Word=1" \
#  --add "spell_Inflict_Wounds=1" \
#  --crop 

#python collect_and_print.py \
#  --cards "./out_cards" \
#  --add "spell_Druidcraft=1" \
#  --add "spell_Thorn_Whip=1" \
#  --add "spell_Cure_Wounds=1" \
#  --add "spell_Charm_Person=1" \
#  --add "spell_Create_or_Destroy_Water=1" \
#  --add "spell_Detect_Magic=1" \
#  --add "spell_Detect_Poison_and_Disease=1" \
#  --add "spell_Entangle=1" \
#  --add "spell_Faerie_Fire=1" \
#  --add "spell_Fog_Cloud=1" \
#  --add "spell_Goodberry=1" \
#  --add "spell_Healing_Word=1" \
#  --add "spell_Jump=1" \
#  --add "spell_Longstrider=1" \
#  --add "spell_Purify_Food_and_Drink=1" \
#  --add "spell_Speak_with_Animals=1" \
#  --add "spell_Thunderwave=1" \
#  --add "spell_Absorb_Elements=1" \
#  --add "spell_Beast_Bond=1" \
#  --add "spell_Earth_Tremor=1" \
#  --add "spell_Ice_Knife=1" \
#  --add "spell_Animal_Friendship=1" \
#  --add "spell_Protection_from_Evil_and_Good=1" \
#  --crop 

#python collect_and_print.py \
#  --cards "./out_cards" \
#  --add "Cloak_of_Billowing=1" \
#  --add "Cloak_of_Dramatic_Entrance=1" \
#  --add "Cloak_of_Convenient_Pockets=1" \
#  --add "Cloak_of_Minor_Applause=1" \
#  --add "Cloak_of_Self-Cleaning=1" \
#  --add "Cloak_of_Cat_Attraction=1" \
#  --add "Cloak_of_Protection=1" \
#  --add "Cloak_of_Elvenkind=1" \
#  --add "Cloak_of_the_Manta_Ray=1" \
#  --add "Cloak_of_Displacement=1" \
#  --add "Cloak_of_the_Bat=1" \
#  --add "Cloak_of_Arachnida=1" \
#  --add "Cloak_of_the_Winter_Wolf=1" \
#  --add "Cloak_of_Invisibility=1" \
#  --crop 

#python collect_and_print.py \
#  --cards "./out_cards" \
#  --add "Potion_of_Healing=9" \
#  --crop 

# python collect_and_print.py \
#   --cards "./out_cards" \
#   --add "Potion_of_Healing=6" \
#   --add "Potion_of_Greater_Healing=6" \
#   --add "Potion_of_Climbing=3" \
#   --add "Potion_of_Growth=3" \
#   --add "Potion_of_Invisibility=3" \
#   --add "Potion_of_Invulnerability=3" \
#   --add "Potion_of_Water_Breathing=3" \
#   --add "Potion_of_Vitality=3" \
#   --add "Potion_of_Speed=3" \
#   --add "Potion_of_Flying=3" \
#   --add "Potion_of_Heroism=3" \
#   --add "Potion_of_Fire_Breath=3" \
#   --add "Cloak_of_Cat_Attraction=1" \
#   --add "Cloak_of_Billowing=1" \
#   --add "Cloak_of_Dramatic_Entrance=1" \
#   --crop 

#python collect_and_print.py \
#  --cards "./out_cards" \
#  --add "Mollys_Cloak_of_Protection=3" \
#  --crop 


python collect_and_print.py \
  --cards "./out_cards" \
  --add "spell_Minor_Illusion=1" \
  --add "spell_Blindness_Deafness=1" \
  --add "spell_Faerie_Fire=1" \
  --add "spell_Grease=1" \
  --add "spell_Entangle=1" \
  --crop 

